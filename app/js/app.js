document.addEventListener("DOMContentLoaded", () => {
	...
	*/
    ...
    function commentSubmit() {
        ...
        // set default input params, after submit
        username.value = "";
        content.value = "";
        date.value = defaultDate.toLocaleDateString("en-CA");
    ...
    function setDate(valueDate) {
        if (defaultDate - valueDate <= 24 * 60 * 60 * 1000) {
            // if inputed date = today's date
            return `Сегодня, в ${padTo2Digits(defaultDate.getHours())}:${padTo2Digits(defaultDate.getMinutes())}`;
        }
        if (defaultDate - valueDate <= 48 * 60 * 60 * 1000) {
            // if inputed date = yesterday's date
